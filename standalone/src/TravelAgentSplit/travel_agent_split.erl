...
record_cc_info(RootCID, CCNumber, ExpiryDate, CVC, State) ->
  CustomerInfo = State#travel_agent_state.customer_info,
  BookingInfo = orddict:fetch(RootCID, CustomerInfo),
  NewBookingInfo = BookingInfo#booking_info{cc_number=CCNumber,
                                            cc_expiry_date=ExpiryDate,
                                            cc_cvc=CVC
                                           },
  NewCustomerInfo = orddict:store(RootCID, NewBookingInfo, CustomerInfo),
  State#travel_agent_state{customer_info=NewCustomerInfo}.
...
calculate_price(CID, BookingInfo) ->
  ...
  error_logger:info_msg("Outward price: ~p, Return price: ~p, Hotel price: ~p~n",
...
record_hotel_success(ConvKey, State) ->
  RootCID = conversation:get_root_cid(ConvKey),
  BookingInfo = get_booking_info(RootCID, State),
  NewBookingInfo = BookingInfo#booking_info{hotel_booked=true},
  NewState = update_booking_info(RootCID, NewBookingInfo, State),
  check_booked(RootCID, ConvKey, NewState),
  NewState.

record_flight_success(ConvKey, State) ->
  RootCID = conversation:get_root_cid(ConvKey),
  BookingInfo = get_booking_info(RootCID, State),
...
ssactor_conversation_established("PerformBooking", _, _, ConvKey, State) ->
  travel_customer_split:cc_info_request(ConvKey),
  {ok, State};
ssactor_conversation_established("PerformPayment", _, _, ConvKey, State) ->
  RootCID = conversation:get_root_cid(ConvKey),
  BookingInfo = get_booking_info(RootCID, State),
  ...
  Price = calculate_price(RootCID, BookingInfo),
  payment_processing_service_split:process_payment(ConvKey, CCNumber,
...
check_received_info(ConvKey, CID, State) ->
  ...
  if ReturnedOutwardFlights =/= undefined andalso ReturnedReturnFlights =/= undefined
     andalso ReturnedHotels =/= undefined ->
       ...
       travel_customer_split:booking_response(ConvKey, ReturnedOutwardFlights,
...
get_booking_info(CID, State) ->
  orddict:fetch(CID, State#travel_agent_state.customer_info).
...
book_flights_and_hotel(CID, ConvKey, State) ->
  ...
  flight_booking_service_split:book_flight(ConvKey, Name, OutwardFlight, ReturnFlight),
  hotel_booking_service_split:book_hotel(ConvKey, Name, HotelName, CheckInDate, CheckOutDate).
...
ssactor_handle_message("BookTravel", "TravelAgent", CID, _SenderRole, "customerRequest",
  ...
  flight_booking_service_split:get_flights(ConvKey, CustomerRequest),
  hotel_booking_service_split:get_hotels(ConvKey, CustomerRequest),
...
ssactor_handle_message("BookTravel", "TravelAgent", CID, _SenderRole, "proceedWithBooking",
  ...
  {ok, NewState};
ssactor_handle_message("PerformBooking", "TravelAgent", _CID, _SenderRole, "ccInfoResponse",
                      [CCNum, ExpiryDate, CVC], State, ConvKey) ->
  RootCID = conversation:get_root_cid(ConvKey),
  NewState = record_cc_info(RootCID, CCNum, ExpiryDate, CVC, State),
  book_flights_and_hotel(RootCID, ConvKey, NewState),
  {ok, NewState};
ssactor_handle_message("PerformBooking", "TravelAgent", _CID, _SenderRole, "flightBookingConfirmation", [],
                       State, ConvKey) ->
  NewState = record_flight_success(ConvKey, State),
  {ok, NewState};
ssactor_handle_message("PerformBooking", "TravelAgent", _CID, _SenderRole, "hotelBookingConfirmation", [],
                       State, ConvKey) ->
  NewState = record_hotel_success(ConvKey, State),
  {ok, NewState};
ssactor_handle_message("PerformPayment", "TravelAgent", _CID, _SenderRole, "paymentConfirmation", [],
                       State, ConvKey) ->
  conversation:subsession_complete(ConvKey, ok),
  {ok, State};
ssactor_handle_message(_, _, _CID, _SenderRole, Op, Payload, State, _ConvKey) ->
  actor_logger:err(travel_agent, "Unhandled message: (~s,  ~p)", [Op, Payload]),
  {ok, State}.

terminate(_, _) -> ok.

% If booking completes successfully, start the payment subsession
