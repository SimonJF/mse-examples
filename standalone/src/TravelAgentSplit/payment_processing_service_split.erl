-module(payment_processing_service_split).
...
ssactor_handle_message("PerformPayment", "PaymentProcessor", _CID, SenderRole, "processPayment",
                       [_CCNumber, _ExpiryDate, _CVC, _Money], State, ConvKey) ->
  travel_agent_split:payment_confirmation(ConvKey),
  {ok, State};
ssactor_handle_message(_, _, _CID, _SenderRole,
                       "cancelBooking", _Payload, State, _ConvKey) ->
  {ok, State};
ssactor_handle_message("PerformPayment", "PaymentProcessor", _CID, _SenderRole,
...
process_payment(ConvKey, CCNumber, ExpiryDate, CVC, Money) ->
  conversation:send(ConvKey, ["PaymentProcessor"], "processPayment", [],
