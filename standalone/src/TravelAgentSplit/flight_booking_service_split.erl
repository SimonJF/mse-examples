-module(flight_booking_service_split).
...
ssactor_handle_message("BookTravel", "FlightBookingService", _CID, _SenderRole,
  ...
  travel_agent_split:send_flight_info(ConvKey, OutwardFlights, ReturnFlights),
  {ok, State};
ssactor_handle_message("PerformBooking", "FlightBookingService", _CID, _SenderRole, "bookFlight",
                       [Name, OutwardFlightID, ReturnFlightID], State, ConvKey) ->
  State1 = add_booking(OutwardFlightID, ReturnFlightID, Name, State),
  travel_agent_split:flight_confirmation(ConvKey),
  {ok, State1};
